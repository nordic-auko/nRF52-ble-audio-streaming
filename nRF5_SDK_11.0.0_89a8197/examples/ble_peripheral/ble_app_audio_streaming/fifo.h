...
static inline bool fifo_put_pkt(fifo_t * p_fifo, uint8_t * p_buf, uint32_t p_buf_len)
{
    ...
    if (p_fifo->end_idx + p_buf_len > sizeof(p_fifo->buf))
    {
        memcpy(&p_fifo->buf[p_fifo->end_idx], p_buf, (sizeof(p_fifo->buf) - p_fifo->end_idx));
        p_buf_len      -= (sizeof(p_fifo->buf) - p_fifo->end_idx);
        p_buf          += (sizeof(p_fifo->buf) - p_fifo->end_idx);
        p_fifo->end_idx = 0; // Wrap around
    }
    
    memcpy(&p_fifo->buf[p_fifo->end_idx], p_buf, p_buf_len);
    
    p_fifo->end_idx += p_buf_len;

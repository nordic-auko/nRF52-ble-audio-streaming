...
int   main(int argc, char **argv)
{
   ...
   if (!strcmp(argv[1],"enc")) 
   {
      ...
      state = allocWord16(0,sizeof(struct BV32_Encoder_State)/2-1);
      Reset_BV32_Encoder((struct BV32_Encoder_State*)state);
   } 
   else 
   {
      sizestate = sizeof(struct BV32_Decoder_State);
      state = allocWord16(0,sizeof(struct BV32_Decoder_State)/2-1);
      Reset_BV32_Decoder((struct BV32_Decoder_State*)state);
   }
   ...
   if (enc){
#if G192BITSTREAM
      fprintf(stderr," BroadVoice32 Fixed-Point Encoder V1.2 with ITU-T G.192\n");
#else
      fprintf(stderr," BroadVoice32 Fixed-Point Encoder V1.2 with packed bit-stream\n");
#endif
...
#if G192BITSTREAM
      fprintf(stderr," BroadVoice32 Fixed-Point Decoder V1.2 with ITU-T G.192\n");
#else
      fprintf(stderr," BroadVoice32 Fixed-Point Decoder V1.2 with packed bit-stream\n");
#endif
   ...
   while (1) 
   {
      ...
      {
#if G192BITSTREAM
         nread = bv32_fread_g192bitstrm((struct BV32_Bit_Stream*)bs, fi);
#else
         nread = fread(PackedStream, sizeof(UWord8), 20, fi);
         BV32_BitUnPack ( PackedStream, (struct BV32_Bit_Stream*)bs ); 
#endif
         if (nread<=0) goto End;
...
#if G192BITSTREAM
         bv32_fwrite_g192bitstrm((struct BV32_Bit_Stream*)bs,fo);
#else
      ...
      {
         if (!bfi) 
            BV32_Decode((struct BV32_Bit_Stream*)bs,(struct BV32_Decoder_State*)state, x);   
         else 
         {
            BV32_PLC((struct BV32_Decoder_State*)state,x); 
         }
